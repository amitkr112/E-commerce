...
export const createOrder = (order) => async (dispatch, getState) => {
    ...
    try {
        //getstate will return all the redux store and from that we extract usersign and from that userinfo
...
export const payOrder = (order, paymentResult) => async (dispatch, getState) => {
    ...
    const { userSignin: { userInfo } } = getState()
    try {
        //we are calling pay api which is of type put hence using put
...
export const listOrderMine = () => async (dispatch, getState) => {
    ...
    try {
        //Ajax request
        // this api is implemented in backend
