...
export const register = (name, email, password) => async (dispatch) => {
    ...
    try {
        ...
        // So that the user remains signed in  even after exiting the browser 
...
export const signout = () => (dispatch) => {
    // Removing the userinfo while signing out
    localStorage.removeItem('userInfo')
    // 
    // Removing the cartitems while signing out
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
    ...
    const { userSignin: { userInfo } } = getState()
    try {
        //Update action hence using put
        const { data } = await Axios.put(`/api/users/profile`, user, {
            headers: { Authorization: `Bearer ${userInfo.token}` }
        })
        dispatch({ type: USER_UPDATE_PROFILE_SUCCESS, payload: data })
        // Since after successful updation we have to show the updated user profile and it comes from signin 
